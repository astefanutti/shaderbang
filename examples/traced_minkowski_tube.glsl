...
// Copied from https://www.shadertoy.com/view/4lSXzh
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    //if(sDist>1e-8){
        ...
        // "bump mapping" above.
        vec3 objCol = pow(min(vec3(1.5, 1, 1)*c, 1.), vec3(1, 3, 16)); // Cheap, but effective, red palette.
        ...
        col = (objCol*(diff + ref*.5 + .25 + vec3(1, .9, .7)*spec) + vec3(1., .8, .9)*ref*.35)*atten;
