...
from libevdev import Device, EV_ABS, EV_KEY, EV_REL
...
class Keyboard(shaderbang.input.Keyboard, Texture):
    ...
    def render(self, frame, **kwargs):
        glsl.glActiveTexture(GL_TEXTURE0 + self.unit)
        glsl.glBindTexture(GL_TEXTURE_2D, self.tex)
        glsl.glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, 256, 3, 0, GL_RED, GL_UNSIGNED_BYTE, self.buffer)
        glsl.glGenerateMipmap(GL_TEXTURE_2D)
...
class Mouse(shaderbang.input.Mouse, Uniform):
    ...
    def render(self, **kwargs):
        ...
        glsl.glUniform4f(self.loc, c_float(x), c_float(y), c_float(z), c_float(w))
...
class Touchscreen(MultiTouch[TouchSlot], shaderbang.input.Touchscreen[TouchSlot], Uniform):
    ...
    def render(self, **kwargs):
        ...
        glsl.glUniform4fv(self.loc, len(self.u4fv), self.u4fv)
