...
from ctypes import *
from typing import Callable, Generic, Iterable, Optional, Protocol, Self, TypeVar, Union, runtime_checkable
...
from libevdev import Device, EV_ABS, EV_KEY, EV_REL, EventCode, EventsDroppedException
...
class FallbackInput(Exception):
    ...
    def __init__(self, fallback: Callable[[], Input]):
...
def _init_input(input: Input, width: int, height: int):
    ...
    def _push_right(f: Callable[[], Input]): i = f(); _pending_inputs.rotate(-1); return i
...
def _update(frame, time):
    try:
        ...
        if len(_pending_inputs):
            viewport = (c_uint*4)()
            sb.glGetIntegerv(GL_VIEWPORT, viewport)
...
class Keyboard(LockableInput):
    ...
    def down(self, key_or_op: Union[EventCode, Callable[[Iterable], bool]], *keys: EventCode) -> bool:
        return self.combo(key_or_op, *keys, state=KeyState.DOWN)

    def pressed(self, key_or_op: Union[EventCode, Callable[[Iterable], bool]], *keys: EventCode) -> bool:
        return self.combo(key_or_op, *keys, state=KeyState.PRESSED)

    def toggled(self, key) -> bool:
        return self.state(key, KeyState.TOGGLED)

    def combo(self, arg: Union[EventCode, Callable[[Iterable], bool]], *keys: EventCode, state: KeyState) -> bool:
        if isinstance(arg, EventCode):
            if len(keys) > 0:
                raise ValueError("only a single key or multiple keys should be passed")
            return self.state(arg, state)
        elif callable(arg):
            if len(keys) == 0:
                raise ValueError("missing keys to pass to the logical operator")
            return arg([self.state(key, state) for key in keys])
